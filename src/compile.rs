...
impl<'a> Compile<'a> for &'a str {
    ...
    fn compile(self, func:&UncompiledFunction<'a>) -> &'a Val {
        unsafe {
            ...
            let offset_data = cast::<_, usize>(slice) - cast::<_, usize>(slice);
            let offset_len = cast::<_, usize>(slice.len()) - cast::<_, usize>(slice);
            func.insn_store_relative(structure, offset_data,
                func.insn_of(mem::transmute::<_, isize>(slice)));
    ...
    fn get_type() -> CowType<'a> {
        ...
        unsafe {
            jit_type_set_size_and_alignment((&ty).into(),
                mem::size_of::<&[u8]>() as i64,
                mem::align_of::<&[u8]>() as i64);
        }
        ty.into()
    }
}
